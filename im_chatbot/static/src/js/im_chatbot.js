odoo.define('im_chatbot.im_chatbot', function (require) {
"use strict";

var local_storage = require('web.local_storage');
var bus = require('web.Bus').bus; // Ivastanin :: var bus = require('bus.bus').bus;
...
//deprecated: var Model = require('web.Model');
var Model = require('web.rpc');

var ChatWindow = require('mail.ChatWindow'); // var ChatWindow = require('im_livechat.WebsiteLivechatWindow');
...
var LivechatButton = Widget.extend({
    ...
    willStart: function () {
        ...
        var ready;
        if (!cookie) {
            ready = session.rpc("/im_chatbot/init", {channel_id: this.options.channel_id}).then(function (result) {
    ...
    open_chat_window: function (channel) {
        ...
        this.chat_window.on("post_message", this, function (message) {
        	...
        	self.send_message(message).then(function () {
        		//chatbot response
            	self.chatbot_response(message).fail(function (error, e) {
                    e.preventDefault();
                    return self.chatbot_response(message);
                })
            }).fail(function (error, e) {
                e.preventDefault();
                return self.send_message(message).then(function () { // try again just in case
            		//chatbot response
                	self.chatbot_response(message).fail(function (error, e) {
                        e.preventDefault();
                        return self.chatbot_response(message);
    ...
    chatbot_response: function (message) {
        var self = this;
        return session
            .rpc("/mail/chatbot_post", {uuid: this.channel.uuid, message_content: message.content, channel_id: this.options.channel_id})
            .then(function () {
                self.chat_window.thread.scroll_to();
            });
    },
